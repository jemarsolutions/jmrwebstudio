...
import { getAllClientsOnDevelopment, updateClientStatus } from "@/server/actions";
import { headers } from "next/headers";
import { redirect } from "next/navigation";
import { Client } from "@/types";

export default async function ClientDevelopmentPage() {
  const clients = await getAllClientsOnDevelopment();
                      ...
                      {clients.length === 0 && (
                        <tr>
                          <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">
                            No clients in development phase.
                          </td>
                        </tr>
                      )}
                      {clients.map((client: Client) => (
                        <tr key={client.id} className="odd:bg-white even:bg-gray-100 dark:odd:bg-neutral-900 dark:even:bg-neutral-800">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            <form action={async () => {
                              "use server";
                              await updateClientStatus(client.id, "Revisions");
                            }} className="inline">
